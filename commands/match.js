...
module.exports = {
            ...
            subcommand
                ...
                .addStringOption((option) =>
                    option
                        ...
                        .setDescription(
                            "Message to be posted on channel on start"
                        )
                        .setMinLength(5)
                        .setRequired(true)
                )
                .addStringOption((option) =>
                    option
                        ...
                        .setDescription(
                            "Message to be posted on channel when timer ends"
                        )
                        .setMinLength(5)
                        .setRequired(true)
                )
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addIntegerOption((option) =>
                    option
                        .setName("id")
                        .setDescription("Timer ID to remake")
                        .setRequired(true)
                )
                .addStringOption((option) =>
                    option
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addIntegerOption((option) =>
                    option
                        .setName("id")
                        .setDescription("Timer ID to stop")
                        .setRequired(true)
                )
        )
    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === "start") {
            ...
            const message_before =
                interaction.options.getString("message_before");
            const message_after =
                interaction.options.getString("message_after");
            ...
            if (entry) {
                ...
                if (message) {
                    ...
                    entry.channel.send(
                        `Remake! Please stay in the same lobby!`
                    );
                }

                interaction.client.match = interaction.client.match.filter(
                    (t) => t.id !== timer
                );

                await interaction.reply(
                    `Timer with ID [**${entry.id}**] remaked!`
                );
            } else {
                await interaction.reply({
            ...
            if (entry) {
                ...
                await interaction.reply(
                    `Timer with ID [**${entry.id}**] stopeed!`
                );
            }
        }
    },
