...
const prepareChannelName = require("../utils/prepareChannelName.js");

module.exports = {
  data: new SlashCommandBuilder()
    .setName("game")
    .setDescription("Commands related to Game Discord Servers.")
    .addSubcommand(
      (
        subcommand //create_category
      ) =>
        subcommand
          .setName("create_category")
          .setDescription("Creates new region category")
          .addStringOption((option) =>
            option.setName("category_name").setDescription("Category name").setRequired(true)
          )
          .addStringOption((option) =>
            option
          ...
          .addBooleanOption((option) =>
            option
              .setName("create_match_info")
              .setDescription("Do you want to create match info channel?")
              .setRequired(true)
          )
          .addBooleanOption((option) =>
            option
              .setName("create_results")
              .setDescription("Do you want to create results channel?")
              .setRequired(true)
          )
          .addBooleanOption((option) =>
            option
              .setName("create_match_start")
              .setDescription("Do you want to create match start channel?")
              .setRequired(true)
          )
          .addBooleanOption((option) =>
            option
              .setName("create_map_streams")
              .setDescription("Do you want to create map stram channel?")
              .setRequired(true)
          )
          .addBooleanOption((option) =>
            option
              .setName("create_support")
              .setDescription("Do you want to create support channel?")
              .setRequired(true)
          )
    )
    .addSubcommand(
      (
        subcommand //create_roles_and_channels
      ) =>
        subcommand
          ...
          .addRoleOption((option) =>
            option.setName("plama_role").setDescription("Player Manager role").setRequired(true)
          )
          .addRoleOption((option) =>
          ...
          .addBooleanOption((option) =>
            option
              .setName("is_voice")
              .setDescription("Do you want to create voice channels instead text channels?")
          )
          .addBooleanOption((option) =>
            option
    ...
    .addSubcommand(
      (
        subcommand //remove_roles_and_channels
      ) =>
        subcommand
          .setName("remove_roles_and_channels")
          .setDescription("Remove roles and channels")
          .addStringOption((option) =>
            option.setName("roles").setDescription("Role names to be removed").setRequired(true)
          )
          ...
          .addBooleanOption((option) =>
            option
              .setName("is_voice")
              .setDescription("Do you want to remove voice channels instead text channels?")
          )
    )
    .addSubcommand(
      (
  ...
  async execute(interaction) {
    if (interaction.options.getSubcommand() === "create_category") {
      ...
      const create_match_info = interaction.options.getBoolean("create_match_info");
      const create_results = interaction.options.getBoolean("create_results");
      const create_match_start = interaction.options.getBoolean("create_match_start");
      const create_map_streams = interaction.options.getBoolean("create_map_streams");
      const create_support = interaction.options.getBoolean("create_support");
      ...
      const roleClient = interaction.guild.roles.cache.find((r) => r.name === "Client");
      const roleAdmin = interaction.guild.roles.cache.find((r) => r.name === "Admin");
      const rolePlama = interaction.guild.roles.cache.find((r) => r.name === "Player Manager");
      ...
      const roleTeamOwner = interaction.guild.roles.cache.find((r) => r.name === "Team Owner");
      ...
      const channelRules = interaction.guild.channels.cache.find((c) => c.name === "📝┃rules");
      const staffArea = interaction.guild.channels.cache.find((c) => c.name === "🔒 Staff Area");

      if (!roleClient) return await interaction.reply(`> *Role **Client** doesn't exist*`);
      if (!roleAdmin) return await interaction.reply(`> *Role **Admin** doesn't exist*`);
      if (!rolePlama) return await interaction.reply(`> *Role **Player Manager** doesn't exist*`);
      if (!roleBroadcast) return await interaction.reply(`> *Role **Broadcast** doesn't exist*`);
      if (!roleCaster) return await interaction.reply(`> *Role **Caster** doesn't exist*`);
      if (!roleTeamOwner) return await interaction.reply(`> *Role **Team Owner** doesn't exist*`);
      if (!roleGuest) return await interaction.reply(`> *Role **Guest** doesn't exist*`);
      if (!roleEveryone) return await interaction.reply(`> *Role **everyone** doesn't exist*`);

      if (!channelLobby) return await interaction.reply(`> *Channel **lobby** doesn't exist*`);
      ...
      if (!channelRules) return await interaction.reply(`> *Channel **📝┃rules** doesn't exist*`);
      if (!staffArea)
      ...
      await interaction.guild.channels
        .create({
          ...
          permissionOverwrites: [
            { id: roleClient.id, allow: [PermissionsBitField.Flags.ViewChannel] },
      ...
      const tag = tag_source ? `${tag_source} - ` : "";
      ...
      await rolesSplitted.forEach(async (role) => {
        let roleToRemove = tag + role.trim();

        if (roleToRemove.startsWith("<@&")) {
          roleToRemove = roleToRemove.substring(3, roleToRemove.length - 1);
        }

        let roleExist =
          interaction.guild.roles.cache.find((r) => {
            return r.name === roleToRemove || r.id === roleToRemove;
          }) || null;

        let channelExist = null;

        if (roleExist) {
          if (is_voice) {
            channelExist = await interaction.guild.channels.cache.find(
              (r) => r.name === roleExist.name
            );
          } else {
            channelExist = await interaction.guild.channels.cache.find(
              (r) => r.name === prepareChannelName(roleExist.name)
            );
          }
        }
        ...
        if (response.length > 1800) {
          interaction.followUp(
            "Removed " + removedChannels + " channels and " + removedRoles + " roles!\n" + response
          );
          response = "";
          removedRoles = 0;
          removedChannels = 0;
        ...
        if (roleExist || channelExist) {
          response += "> ";
        }

        if (channelExist) {
          response += "Channel (#" + channelExist.name + ") removed \t";
          await channelExist.delete().then(removedChannels++);
        }
        if (roleExist) {
          response += "Role (@" + roleExist.name + ") removed";
          await roleExist.delete().then(removedRoles++);
        }

        if (channelExist || roleExist) {
          response += "\n";
        }
      });
      ...
      teamNamesSplitted.forEach((name) => {
        ...
        let channelName = is_voice === true ? fullName : prepareChannelName(fullName);
  ...
  async autoload(client) {
    client.on("messageCreate", async (msg) => {
      if (msg.channel.name == "role-request") {
        ...
        if (author) {
          try {
            let matches = getDisplayName(author).match(/\[(.+)\](.+)/);

            if (
              matches === null &&
              !author.user.bot &&
              !author.permissions.has(PermissionFlagsBits.ManageMessages)
            ) {
              msg
                .reply(
                  `Your nickname on this server (**${author.displayName}**) is not valid. You will not get a role. Please fix your nickname to have it in format \`[TAG] nickname\` and request role again.`
                )
                .then((t) => {
                  setTimeout(() => {
                    msg.delete().catch((err) => {
                      console.log("[4nfg] Message already removed... ", err.message);
                    });
                  }, 500);

                  setTimeout(() => {
                    t.delete().catch((err) => {
                      console.log("[hfd5] Message already removed... ", err.message);
                    });
                  }, 30000);
                });
            }
          } catch (err) {
            console.log("[n45f] Something wrong :( ", err.message);
          }
        }
      }
    });
